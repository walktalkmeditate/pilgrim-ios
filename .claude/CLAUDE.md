...
- **Frozen CoreStore string identifiers**: The `static let identifier = "..."` strings on every `PilgrimV*` / `OutRunV*` entity (e.g. `"Workout"`, `"WorkoutPause"`, `"WorkoutRouteDataSample"`, `"WorkoutHeartRateSample"`) map directly to SQL entity names in the shipped database. Same for `Relationship.ToOne<...>("workout")`, `Relationship.ToManyOrdered<...>("workouts"/"workoutEvents"/etc.)`, and `Value.Required<...>("workoutType"/etc.)` — those string arguments are SQL column/relationship names. Changing any of them on an existing legacy entity would require a schema migration. The *Swift class names* (e.g. `PilgrimV7.Walk`, `TempV4.WalkPause`) are labels only — they were renamed off the historical "Workout" prefix to match the app's public vocabulary, and the string identifiers intentionally stayed behind as frozen SQL names. For brand-new entities introduced in the current schema version, you have full freedom to name both the class and the identifier string consistently.
