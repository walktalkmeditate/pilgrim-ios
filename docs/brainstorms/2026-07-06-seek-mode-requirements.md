...
- R4. Clearing count (1–3) is drawn randomly within a band derived from duration (longer walks tilt toward more clearings; adjacent presets share overlapping bands) and never shown to the walker, so a repeat walker can never infer the count from the preset. The duration budgets a one-way journey: clearings are strung outward along a wandering line, with the final clearing landing near the walking limit — there is no return leg, and the way home belongs to the walker. Generation assumes a contemplative pace (~24–25 min/mile), reserves roughly a quarter of the time for stillness and exploration, and converts the walking budget to straight-line reach with a street-winding allowance (~1.25×). Duration is a generation input only: nothing happens when the chosen time elapses mid-walk — no timer, no signal — the pulse simply continues until the walker ends the walk.
