...
> **Superseded in part by the second review pass (G2, G3):** the rendered line is now "On every walk where '_' appears, the speech carrying it was Y-dominant" — the evidence is the theme-carrying recordings, not the walk, so the sentence names them. `dominantModalFamily` additionally requires `frameConstancyMinModalsPerWalk` mapped tokens and returns nil on a top-two tie. Every "Every walk where X appears is Y-dominant" string below, and Deviation 2's claim that this mirrors `modalLeanSummary`, is stale.
...
>
> **Superseded again by the second review pass (G1):** both the evidence and the denominator are now bounded by `ThreadStore.recurrenceWindow`, the window `ThreadsDossierBuilder.resolveFixes` itself draws from, and both variants name it — "on all N of its walks in the last 30 days" / "on M of its N walks in the last 30 days". Every walk count below that reads from the theme's full unwindowed appearance history is stale. The signal also moved to `DossierSensesInvariancePlace.swift` and is English-gated (G9).
...
**Explicitly not fixed, by instruction.** F5 (no repetition guard — invariants do not move, so consecutive walks receive identical lines) needs a product decision on cadence, mirroring `moonLine`'s once-per-lunation budget, and is the owner's call. F9 (`ThreadsDossierFormatter`'s `includeThreadAnalysis` parameter is dead) was left alone — **superseded by G6 below, which removed it.**

---

## Second review pass — 10-persona adjudication (2026-08-28)

Three more false-claim bugs of the same family the first pass fixed six of, plus the gaps that let them through. Everything below is live; drafts above are read through these.

**G1 — `placeFrameLock`'s denominator is now bounded by the same window its evidence is (Task 6, supersedes F8's counts).** `ThreadsDossierBuilder.resolveFixes` resolves a route fix only for an appearance whose recording instant falls inside `ThreadStore.recurrenceWindow`, but `allWalks` was built from the thread's ENTIRE unwindowed appearance history. A theme spoken on 14 walks over six months, at fourteen carefully logged places, rendered *"on 3 of the 14 walks it appears in"* — eleven of those walks were never offered to the locator at all, so the sentence contrasted what was measured against a set that could not have been. The signal now filters appearances to the window before it counts anything, and both variants name the window out loud: "on all 3 of its walks in the last 30 days" / "on 3 of its 4 walks in the last 30 days". `placeResonance` and `intentionLineage` already windowed both sides; this follows them. **The doc comment blaming rural GPS coverage for the measured-vs-total gap was wrong and is corrected** — rural coverage is the reason the signal gates on the measured set rather than demanding full coverage *within* the window, which stands; it was never the reason the two counts diverged by eleven. Pinned by `testPlaceFrameLock_appearancesOutsideTheFixWindow_areNotInTheDenominator`; every previously pinned string updated.

**G2 — `frameConstancy` no longer makes a walk-scoped claim from theme-scoped evidence (Task 5).** It sums `modalCounts` over `thread.appearances` — only the recordings that carried the theme — and then said *"Every walk where 'X' appears is Y-dominant"*, a claim about the WALK. Multi-recording walks are ordinary, so a walk can be counterfactual-dominant overall while the two recordings that mentioned 'money' are obligation-dominant. Re-scoped, not re-measured: **"On every walk where 'money' appears, the speech carrying it was obligation-dominant."** Same move F8 made for `placeFrameLock`, same precedent `markerColoring` sets with its ±15-token window. The doc comment claiming this mirrors `ThreadsDossierFormatter.modalLeanSummary` was wrong — that one sums ALL of a walk's recordings; only the per-walk summing *move* is shared, and the comment now says so. Pinned by `testFrameConstancy_claimsTheSpeechItMeasured_notTheWholeWalk`; two previously pinned strings updated.

**G3 — `dominantModalFamily` gets the gates its sibling has (Task 5).** It copied `modalLeanSummary`'s tie mechanics — declaration order over `ModalFamily.allCases`, strict `>` to replace — while dropping the two gates that make them field-safe there: `modalRemarkableMinCount` (10) *and* 2× baseline elevation. With only `count > 0`, a single "can" made a walk possibility-dominant, and `possibility` leads `allCases`, so it won every tie. Same shape as `weatherWeave`'s plurality tautology (fixed by the majority→mode guard in PR #71) and `questionDensity`'s unfloored count (cut at the ship gate for firing 151 times on Whisper punctuation). Two additions: **`frameConstancyMinModalsPerWalk = 5`**, and **a top-two tie returns nil** so a tie silences rather than silently electing `possibility`. The floor is deliberately below the sibling's 10 — that gate weighs a walk's *entire* speech and stands alone as a single-walk claim, while this one weighs only the theme-carrying subset and the sentence it guards also requires agreement across ≥ `minimumInvariantWalks` walks, so one rendered line rests on ≥ 3 × 5 = 15 modal tokens; a floor of 10 on a theme-scoped subset would have made the signal unfirable. Pinned by `testFrameConstancy_oneModalPerWalk_isNotAFrame`, `testFrameConstancy_atTheModalFloor_speaks` (the floor must not be a mute button), and `testFrameConstancy_topTwoFamiliesTie_holdsTheSignalSilent`.

**G4 — one integration test composes a real block (Task 2).** Every `invarianceLines` test drove the stub `evaluate` seam, so nothing ever built a real multi-signal `Unchanged:` block from real signals through the production dispatch — which is how G1 and G7 both survived a green suite. `testInvarianceLines_realSignals_composeTheUnchangedBlock` fires `fusedThemes` and `placeFrameLock` on different themes from one `Input` and asserts the whole composed block string, `**Unchanged:**` heading included.

**G5 — the context policy no longer leaks its guard (Task 9, corrects F6's premise).** Creative and Gratitude get the senses-only dossier because their policy sets `includesThreadAnalysis: false` — but `placeResonance`, `weatherWeave`, `moonLine` and `intentionLineage` all name a theme and count walks *inside* `Noticed:`. They received cross-walk thread claims either way, and the clinical-language guard was keyed on the policy rather than on the block actually selected, so those claims arrived with no instruction on how not to read them. **The guard now follows the selected dossier**, whichever variant that is. `interpretiveKey` still teaches nothing here: its probes look for marker phrasings the senses-only variant never prints, so the accretion is the guard alone and vocabulary-with-no-referent stays out. Chosen over filtering theme-naming senses out of `sensesOnly`, which would have gutted the block — nearly every sense names a theme, and 847ca7b exists precisely to give these two voices their weather back. `testAssembler_creativeAndGratitude_contractStillCarriesNoThreadsLanguage_withSensesOnlyDossier` pinned the defect and is replaced by `testAssembler_creativeAndGratitude_sensesOnlyDossier_carriesTheClinicalGuard`.

**G6 — `ThreadsDossierFormatter.dossier`'s `includeThreadAnalysis` parameter removed (was F9).** Never passed `false` by any caller, tests included: the thread-suppressed voices never call the formatter at all, because the builder assembles `dossierSensesOnly` from the `Noticed:` block directly. The parameter and its branch are gone; a comment at the branch site records why there is no switch.

**G7 — `fusedThemes` reserves both themes it names (Task 3).** It prints two display terms but inserted only the anchor lemma into `used`, so a lower-ranked signal could go on to make its own measured claim about the superset — a second, independent-looking finding about a theme the first line had already spoken for. `DossierSenses.SenseLine` gains `secondaryLemma` and `claimedLemmas`; both engines (`lines` and `invarianceLines`) now check and reserve every lemma a line names, not just the anchor. Pinned by `testFusedThemes_reservesBothThemesItNames` and `testEngine_secondaryLemmaOfAHigherRankedLine_suppressesLowerRank`.

**G8 — sentiment flatness is judged on an absolute band, not a coefficient of variation (Task 4).** CV over the +1.0-shifted scale is asymmetric: `{0.9, 0.5, 0.7}` reads 0.096 (flat) and `{-0.9, -0.5, -0.7}` reads 0.544 (not flat) despite an identical standard deviation. Positive themes earned "it sounds the same each time"; identically-varying negative ones were refused it — the one direction this app must never get wrong. Sentiment's zero is a midpoint, not an absence, so there is no denominator to divide by. `sentimentShift` is gone; `isSentimentFlat` judges standard deviation against **`sentimentFlatnessBand = 0.20`**, exactly the spread the CV ceiling already granted at the scale's midpoint, so the change corrects the asymmetry without moving the bar. CV stays for the two count densities, which do have a meaningful zero. Pinned by `testSentimentFlatness_isSignSymmetric`, `testSentimentFlatness_steadyMoodIsFlat_swingingMoodIsNot`, and `testUnmovedReturn_steadilyNegativeSentiment_stillSpeaks`.

**G9 — `fusedThemes` and `placeFrameLock` are explicitly English-gated (Tasks 3 and 6).** The other three signals read a `MarkerPack`, which `MarkerAnalyzer.compute` returns nil for outside `"en"`, so they were English-gated by accident. These two read only themes and coordinates and were gated by nothing — while `TranscriptNLP.contentLemmaMentions` falls back to the lowercased surface form wherever NLTagger has no lemma model. This app's walkers cross the Camino in French, German, Italian, Portuguese and Spanish, where two inflections of one word become two themes: *"'marche' and 'marches' have appeared in the same 4 walks, never apart"* is a tautology about a single word dressed as a discovered fusion. `englishRecordings` / `isEnglishThroughout` gate both, fail-closed. The whole track now speaks under one stated language contract instead of an accidental one. Pinned by `testFusedThemes_nonEnglishRecordings_staySilent` and `testPlaceFrameLock_nonEnglishRecordings_staySilent`.

**G10 — the empty-prompt sentinel is visible at the type level.** `PromptAssembler.assemble` returns `""` for a hoisting voice with no block, but `GeneratedPrompt.text` is non-optional, so `PromptDetailView`'s copy, share and body all used it unguarded with only `PromptListView`'s picker gate in between. `GeneratedPrompt` gains `hasText` and `emptyBodyCopy`; the detail view renders the fallback and disables both actions. Pinned by `testGeneratedPrompt_refusedAssembly_reportsNoText`.

**G11 — the waiting row's copy is readable (Task 11, completes F10).** Measured against `parchment`: `.fog` is 3.4:1 at full strength and 1.6:1 at the row's 0.45 opacity; `.ink` falls from 13.5:1 to 2.6:1. Both far under WCAG AA's 4.5:1, on the one row whose copy exists to be read. The dim now falls on the icon (decorative, exempt, still 0.45) and on a softened `.ink` at 0.7 — 5.4:1 light, 7.8:1 dark. Unavailability is carried by the icon dim, the missing chevron, the copy itself and F10's accessibility label. Pinned by `testWaitingRow_textDimStaysAboveTheAccessibleContrastFloor`.

**G12 — the waiting copy stops promising something walking cannot deliver (Task 11).** "Still listening. A few more walks with your voice." is false when `UserPreferences.threadsAfterWalks` is off: `buildResult` returns all-nil before reading a single recording, so the row would sit dimmed forever telling the walker to keep doing the one thing that cannot change it. `waitingCopy` takes `threadsEnabled` and returns "Needs Thought Threads, switched on in Settings → Voice." in that case. Pinned by `testWaitingCopy_threadsDisabled_pointsAtTheSettingNotAtMoreWalking`.

**G13 — the memo's cache-hit path is pinned on all four fields.** The memo caches a four-field tuple, but every test read `.dossier` through `build`, which discards the other three. A hit that dropped `unchangedBlock` would show up as Oblique silently vanishing from the picker on the second open of a walk, and nothing would have failed. `testBuilder_cacheHit_replaysEveryFieldNotJustTheDossier` asserts the hit replays the miss field for field, and proves the fixture actually hit the memo.

**G14 — the invariance field report prints its gates and its cost (Task 12).** It printed firing rates only, which answers "what would each signal say across all walks" but never "why did Oblique say nothing on this walk" — and this harness bypasses the production dispatch, so every gate can be closed while every rate looks healthy. It now prints `UserPreferences.threadsAfterWalks`, `ThreadsBackfill.isComplete`, `DossierSensesInvariance.pendingFieldGate`, the count of recordings with no stored context (and the walks they sit on), and per-walk evaluation timing with a median/max summary — the same discipline `generate()` already applied to senses.

**Not fixed, and why.** **`backfillComplete` is the wrong gate for coverage claims.** It certifies *analysis* coverage of already-transcribed recordings, not transcription coverage: `ThreadsBackfill` sweeps `transcribedRecordingsSnapshot()` only, and `autoTranscribe` defaults off, so a device with 32 of 40 walks untranscribed still reads `isComplete == true` and every "every walk" claim silently means "every walk we happen to have words for". Strengthening it to require full transcription coverage is a one-line change in `buildResult` — the data is already in hand (`walkIndex.keys` minus `contextsByUUID.keys`) — but with auto-transcribe off by default it would silence Oblique for the majority of walkers, permanently, on the first recording they never transcribe. That is a decision about the feature's reach, not a defect fix, and the alternative (re-scoping all five sentences to "of the walks whose words were transcribed") is a wording change across every signal that needs the owner's hand. **G14 prints the number the decision needs** — recordings with no stored context, and the walk count they cover — so the field gate can answer it from real history rather than argument.

**File splits forced by the `file_length` gate (second pass).** `placeFrameLock` and `maxPairwiseSpread` moved verbatim (plus G1's changes) to `Pilgrim/Models/Threads/DossierSensesInvariancePlace.swift`, the same split `DossierSensesInvarianceIntention.swift` already took; 4 hand-added `project.pbxproj` entries. `thread(_:walks:salience:)` widened from `private` to internal for the split test files, and `plainContext` lives in `DossierSensesInvarianceEvidenceGatesTests.swift` rather than on the parent class, which sits against `type_body_length`.
