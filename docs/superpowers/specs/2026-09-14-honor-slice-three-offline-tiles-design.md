...
3. **Zoom band: Streets and terrain-v2 at z11–14; no DEM.** Measured on three Francés stages: z14 tiles already carry footpaths, tracks, hamlets and lodging POIs; z15–16 adds building footprints and nothing else. The floor was 0 in the first build, and the first device save — the four-stage Nakahechi, estimated at ~2 MB — landed as **596 MB**. Measured against the tile API on 2026-09-15: the SDK downloads whole tile packs in zoom bands 0–5 / 6–10 / 11–14, not the tiles a corridor touches, so a range starting at 0 pulls the planet-wide 0–5 pack of every tileset in the style (206 MB before a single route tile) and a ~90 MB z6 pack per tileset touched. The z11 band is the first whose packs follow the corridor. The DEM left the region with the floor: its z11 packs are 8–9 MB each — 511 MB across the Francés on their own — for a hillshade, while the style's composite source already carries `mapbox-terrain-v2`, whose contour lines and landcover stay offline. Owner decision, 2026-09-15: z11–14 for both style descriptors, nothing named in `tilesets`. Measured outcome: Camino Francés ~240 MB in 124 packs (62 z11 cells × 2 tilesets), Nakahechi ~8 MB. Offline, below z11 the whole-route preview draws its line on bare parchment, and the hillshade renders only with signal.
4. **Cellular is allowed; the size is the guardrail.** No wifi-only mode. The button carries the estimate; the walker decides. (Rejected: `NetworkRestriction.disallowCellular` with an override — one more state to draw. At ~240 MB for the Francés the number on the button is the whole guardrail, which is why §2.2 counts what the store actually downloads.)
...
- **Tile pack** — Mapbox's unit of download: a parent tile and its descendants, per tileset, fetched whole whenever a region's geometry touches any of it. Ours are rooted at z11 and run to z14. Regions share packs; the account limit is on *unique* packs.
...
func estimateBytes(for routeId: String, stages: [Way]) -> Int  // z11 pack count of the whole corridor × the route's own bytes-per-pack
...
- Streets: `TilesetDescriptorOptions(styleURI: .light, zoomRange: 11...14, tilesets: nil)` and the same for `.dark`. The light and dark styles read the same composite source — `mapbox-streets-v8`, `mapbox-terrain-v2`, `mapbox-bathymetry-v2` — so the tile store holds each tile once, and terrain-v2 is what keeps contour lines and landcover offline.

Nothing is named in `tilesets`. The first build had a third descriptor for `mapbox://mapbox.mapbox-terrain-dem-v1`, the source `PilgrimMapStyle.applyWabiSabiStyle` adds at runtime for the hillshade; it is gone. DEM tiles run 105–140 KB each at z11–14, so its z11 packs are 8–9 MB apiece and the Francés would carry 511 MB of hillshade alone. Offline the hillshade is absent; with signal it renders as it does today.

**Packs are the download unit, and the band is 11–14.** Tile packs are loaded in fixed zoom bands — 0–5, 6–10, 11–14, 15–16 — and a region pulls every pack its geometry touches, whole; the SDK recommends choosing ceilings on a band edge, and a tile store holds at most 750 unique packs. The first build's range started at 0 so the route page's whole-stage fit at ~z11 would be covered. That made every region pull the planet-wide 0–5 pack of every tileset in the style — 206 MB before a single route tile, measured 2026-09-15 — plus a ~90 MB z6 pack per tileset touched, and the four-stage Nakahechi saved 596 MB against a ~2 MB estimate. The z11 band is the first whose packs follow the corridor, so the range starts there: below z11 the offline preview draws the stage line on bare parchment, and the walk screen's z16 overzooms the saved z14 tiles; online it fetches z16 as it does today. The 15–16 band was dropped because it adds building footprints only (decision 3) and would put the Francés at ~1,800 z15-rooted packs against that 750-pack ceiling; at 11–14 the whole route is 124 packs.
...
`estimateBytes(for:)` counts the distinct z11 tiles the whole route's corridor touches — every stage's rings in one sweep, so a cell two stages share is counted once, as the store holds it once — and multiplies by a bytes-per-pack figure.

**Packs, not tiles.** The first build counted the tiles the corridor touched at z10–14, once for Streets and once for the DEM, at 10 KB each. That is not what the store downloads: it fetches whole packs — a z11 tile and every descendant to z14, per tileset — whenever the corridor touches any of them, and a pack is megabytes where a tile is kilobytes. The ~2 MB Nakahechi estimate was 596 MB on the phone. One z11 cell is the unit the estimate can be honest in.

**The bytes-per-pack figure is per route, never global.** Density differs between routes the catalog already carries — 3.8 MB per z11 cell on the Francés, 2.7 MB on the Nakahechi, both tilesets together — so one shared number calibrated by whichever route saved last would misstate the next route right where the estimate is the only guardrail decision 4 relies on. Each route id gets its own value under `UserDefaults` key `pilgrimage.tiles.bytesPerPack.<routeId>`:

- **Seeded** at 4 MB per z11 cell, the measured Francés figure rounded up, for every route until it has a measurement of its own. At the seed the whole Francés estimates at ~248 MB (62 cells, from the dataset's 33 stage lines) against ~240 MB measured, and the Nakahechi at ~12 MB against ~8 MB.
- **Calibrated** when a save of that route completes: the tile store's real `completedResourceSize` for the route's regions, divided by their pack count, replaces the seed — written only when both are at least 1. After the first save of a route its number stops being a guess; a different route's save never touches it.

The UI always writes the estimate as "~240 MB". Once saved it shows the real byte count with no tilde.
...
do:
    ...
    for stage in stages (by index):
        ...
        if regions()[stage.id] is complete
           and regions()[stage.id].metadata.corridorHash == hash(regionVersion, corridor): done += 1; continue
        loadRegion(id: stage.id, options: TileRegionLoadOptions(geometry: corridor, descriptors: [light, dark],
                                                                 metadata: ["corridorHash": hash(regionVersion, corridor)], acceptExpired: true))
        done += 1
    phase = .idle
catch error:
...
- **"Complete" is not enough to skip a stage — the corridor has to match too.** Each region's `metadata` carries a hash of the corridor it was loaded for. After an Update redraws a stage, its region is still complete by resource count but its hash no longer matches, so the loop reloads it; a region loaded under an existing id is replaced. Without the hash, a resumed save would skip a stage whose offline tiles now cover the wrong path. The hash is SHA-256 over `PilgrimageTilesDescriptors.regionVersion` first and the corridor's coordinates after it. The version is there because the descriptors changed after regions were saved in the field: a first-build region — z0 with the DEM — is complete by resource count and covers the right corridor, and without the version a save would skip it and 596 MB of the wrong packs would keep reading as *maps saved*. With it the region fails the check, the next save reloads it under the current descriptors, and the store frees the packs nothing references any more.
...
- The SDK downloads in-process. There is no background `URLSession`; backgrounding the app mid-save pauses it, and resume covers the rest. At ~240 MB worst case a save is a minute or two on wifi.
...
| `.none` | button: **Save maps for the way · ~240 MB** |
| `.partial(12, of: 33)` | button: **Save maps for the way · 12 of 33 saved** |
| `.saved(bytes)` | check glyph, caption: **maps saved · 240 MB**, tappable to save again |
...
- detail **Camino Francés · 240 MB** otherwise (the route's display name from the installed package, bytes from the store)
...
2. **It is excluded from iCloud backup.** ~240 MB of re-downloadable tiles should not sync. If the SDK does not set `isExcludedFromBackup`, the app does, after the store is created.

Neither the package's `maxPackageBytes` (a JSON ceiling) nor a tile store `DiskQuota` applies to the save. With one pilgrimage at a time and a ~240 MB ceiling across the whole catalog, there is nothing to quota. The ambient cache keeps the SDK's defaults.
...
- **Estimate:** the pack count is the z11 cells of the whole corridor, every stage's rings in one sweep, and a cell two stages share counts once; the estimate scales with `bytesPerPack`; a completed save updates `bytesPerPack` from the real byte count and writes nothing without bytes and packs.
- **The band:** both style descriptors run 11–14 and name no tileset; the pack root is the range's floor; the region version is 2, a corridor hash changes with it, and a region saved under version 1 reads as unsaved.
