...
class HealthStoreManager {
    ...
    class HealthType {
        ...
        /// A static constant containing all HealthKit types implemented and thereby usable in Pilgrim.
    ...
    static func saveHealthWalk(for workout: WalkInterface, completion: @escaping (HealthError?, HKWorkout?) -> Void) {
        ...
        gainAuthorisation(for: [HealthType.Workout]) { workoutAuthorisation, _ in
            ...
            let workoutEvents = createWalkEvents(from: workout)
    ...
    static func saveAllWorkouts(completion: @escaping (_ error: HealthError?, _ allSavedAlready: Bool) -> Void) {
        ...
        let unsavedWorkouts: [Walk] = DataManager.queryObjects(from: \._healthKitUUID == UUID())
     ...
     */
    private static func createWorkoutRoute(from workout: WalkInterface, andAttachTo healthWorkout: HKWorkout) {
     ...
     */
    private static func createHeartRateSamples(from workout: WalkInterface) -> [HKQuantitySample] {
    ...
    static func createHealthWalk(from hkWorkout: HKWorkout) -> HealthWorkout? {
        ...
        let heartRateMapper: ([TempHeartRateDataSample]?, HKQuantity, DateInterval) -> [TempHeartRateDataSample] = { lastValue, quantity, timeInterval -> [TempHeartRateDataSample] in
