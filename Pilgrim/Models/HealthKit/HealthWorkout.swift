...
class HealthWorkout: TempWalk {

    let hkWorkout: HKWorkout

    init?(_ hkWorkout: HKWorkout, steps: Int?, route: [CLLocation], heartRates: [TempHeartRateDataSample]) {

        guard
            let distance = hkWorkout.totalDistance?.doubleValue(for: .meter()),
            let type = Walk.WalkType(hkType: hkWorkout.workoutActivityType)
        else {
            return nil
        }
    ...
    required init(from decoder: Decoder) throws {
        fatalError("[HealthWorkout] Subclass 'HealthWorkout' of TempWalk does not support protocol 'Codable'")
    }

    // MARK: Static

    static func durationTouple(from pauses: [TempWalkPause], startDate: Date, endDate: Date) -> Computation.DurationTouple {
    ...
    static func pauses(from workoutEvents: [HKWorkoutEvent], startDate: Date, endDate: Date) -> [TempWalkPause] {
        ...
        let pauseTouples = Computation.calculateAndValidatePauses(from: events, walkStart: startDate, walkEnd: endDate) ?? []
    ...
    static func workoutEvents(from hkWorkoutEvents: [HKWorkoutEvent]) -> [TempWalkEvent] {

        return hkWorkoutEvents.compactMap {
            guard !$0.type.isPauseType, let eventType = WalkEvent.EventType(healthType: $0.type) else { return nil }
