...
enum SealGenerator {
    ...
    static func generate(from input: SealInput, size: CGFloat = 512) -> UIImage {
        ...
        let hash = SealHashComputer.computeHashFromInput(input)
        let bytes = SealHashComputer.hexToBytes(hash)

        let activeDuration = input.activeDuration
        let meditateRatio = activeDuration > 0 ? input.meditateDuration / activeDuration : 0
        let talkRatio = activeDuration > 0 ? input.talkDuration / activeDuration : 0
        ...
        let renderInput = SealRenderer.Input(
            geometry: geo,
            color: color,
            season: season,
            year: year,
