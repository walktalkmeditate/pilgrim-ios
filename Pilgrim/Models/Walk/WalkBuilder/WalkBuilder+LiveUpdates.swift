...
extension WalkBuilder {
    ...
    /**
     Performs a live update action on the main thread only if appropriate looking at background update suspension and if the `WalkBuilder` even has a delegate
     - parameter closure: the update action that is supposed to be performed
     */
    private func performIfAppropriate(closure: @escaping (WalkBuilderDelegate) -> Void) {
    ...
    */
    ...
    /**
     Live updates the status of the `WalkBuilder` by sending it to the delegate
     */
