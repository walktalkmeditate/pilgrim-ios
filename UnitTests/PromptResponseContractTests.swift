...
final class PromptResponseContractTests: XCTestCase {
    ...
    /// `DossierSenses.markerColoring` emits "Absolutist words cluster around
    /// 'X'" into the `Noticed:` block — capital A — and the share probe looks
    /// for lowercase "absolutist words". `contains` is case-sensitive, so they
    /// do not collide today, and the whole distinction rests on that.
    ///
    /// Lowercasing either string would make a small-sample walk that merely
    /// mentions marker clustering read as though it printed shares, and the
    /// model would weigh a 2-in-40-words tally as a rate — the exact misread
    /// the tally clause exists to prevent.
    ///
    /// The sense line is generated by the real `markerLine`, not copied, so a
    /// casing change in production fails here rather than drifting past a
    /// hardcoded fixture. Fixture shape borrowed from
    /// `DossierSensesMarkerPhotoTests.coloredTranscript`.
    func testInterpretiveKey_markerColoringLine_doesNotMasqueradeAsPrintedShares() throws {
        try XCTSkipUnless(NLAssetAvailability.lemmaAvailable,
                          "markerLine tokenises through the tagger")
        let prefix = "the move must always happen and everything about the move is completely certain now "
        let neutral = "walking along the river path watching herons drift over quiet water today "
        let distant = "walking along the river path watching herons drift over quiet water always "
        let text = prefix + String(repeating: neutral, count: 4) + String(repeating: distant, count: 2)

        var mentions: [ThemeMention] = []
        var search = text.startIndex
        while let range = text.range(of: "move", range: search..<text.endIndex) {
            mentions.append(ThemeMention(start: text.distance(from: text.startIndex, to: range.lowerBound),
                                         length: 4))
            search = range.upperBound
        }
        let theme = Theme(lemma: "move", displayTerm: "the move", mentionCount: 2,
                          salience: 0.05, mentions: mentions)
        let senseLine = try XCTUnwrap(
            DossierSenses.markerLine(theme: theme, displayTerm: "the move", text: text),
            "fixture must actually produce a markerColoring line for this to pin anything"
        )
        XCTAssertTrue(senseLine.contains("Absolutist words"), "guards the capital-A precondition")

        let dossier = smallSampleDossier() + "\n\n**Noticed:**\n" + senseLine
        let contract = PromptAssembler.responseContract(
            voice: ReflectiveVoice(), hasSpeech: true, threadsDossier: dossier
        )
        XCTAssertTrue(contract.contains("do not weigh them"),
                      "only raw counts were printed, so the tally clause must appear")
        XCTAssertFalse(contract.contains("absolutist-word share"),
                       "no recording printed a share; the sense line must not be mistaken for one")
    }
}
